...
open class GenPagesUserFollowListUserFollowList : BasePage {
    ...
    override fun `$render`(): Any? {
        ...
        return createElementVNode("scroll-view", utsMapOf("style" to normalizeStyle(utsMapOf("flex" to "1"))), utsArrayOf(
            if (isTrue(!_ctx.isFirstLoad && _ctx.userList.length == 0)) {
            ...
            ,
            createElementVNode(Fragment, null, RenderHelpers.renderList(_ctx.userList, fun(item, index, _, _): VNode {
                return createVNode(_component_user_list_item, utsMapOf("key" to index, "item" to item), null, 8, utsArrayOf(
                    "item"
                ));
            }
            ), 128),
    ...
    override fun data(): Map<String, Any?> {
        return utsMapOf("user_id" to 0, "loading" to false, "userList" to utsArrayOf<UserItem>(), "isEnded" to false, "currentPage" to 1, "isFirstLoad" to true);
    }
    override fun `$initMethods`() {
        ...
        this.loadData = fun(loadComplete: (() -> Unit)?) {
            ...
            this.loading = true;
            uni_request<Result<UserItemResult>>(RequestOptions(url = getURL("/user/" + this.user_id + "/follows/" + Math.floor(this.currentPage)), header = object : UTSJSONObject() {
                var token = getToken()
            }, success = fun(res){
                var r = res.data;
                if (r == null) {
